...
void dma_setup_mem_copy(DmaChannel *channel, void *dest, void *src, u32 length, u32 burst_length) {
    channel->block->transfer_info = (burst_length << TI_BURST_LENGTH_SHIFT) // How many data words shuold be sent in 1 burst, important for speed
    ...
    channel->block->mode_2d_stride = 0; // 2d stride refers to how memory is copied. 1 means we copy memory in rows and columns
...
void dma_start(DmaChannel *channel) {
    // Sets DMA control block address (source, destination, transfer info, etc.) using the physical bus address
